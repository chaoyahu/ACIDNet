...
opt = option("standard").parse_args()
...
def checkpoint(epoch):
    out_dir = "./outputs/checkpoints/train"
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    model_out_path = os.path.join(out_dir, "epoch_{}.pth".format(epoch))
...
if __name__ == '__main__':  
    ...
    if opt.dry_run:
        print("Dry run passed: data loaders, model, optimizer, scheduler, and losses initialized.")
        raise SystemExit(0)
